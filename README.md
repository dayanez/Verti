...
- **Real tree-sitter highlighting** (`pkg/highlight`) — Go, Python,
  JavaScript/JSX, TypeScript/TSX, Rust, C/C++, Lua, and Markdown are parsed
  into a real AST and classified using node type *and* parent context (so a
  function name colors differently from a plain variable), not regex. JSON
  is covered by a small hand-written tokenizer, since tree-sitter-json has
  no bundled Go grammar.
...
- **No OS clipboard integration.** `Ctrl+C` currently quits the editor
  (there's nothing to copy to yet) rather than copying, unlike VS Code.
  Adding real clipboard support is a natural next step.
...
- No multi-cursor, no find/replace UI, no "Save As" — single-buffer,
  single-file editing for now.
