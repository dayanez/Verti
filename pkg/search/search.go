...
// predictable), skipping dot-directories (.git and the like) and any
// file that's too large or looks binary (contains a NUL byte).
func Files(root, substr string) ([]Match, error) {
	...
	err := filepath.WalkDir(root, func(path string, d fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return nil // an unreadable entry shouldn't abort the whole search
		}
		if d.IsDir() {
